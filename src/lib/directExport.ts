...
/** Полный набор столбцов шаблона Директ Коммандера — все 55 по порядку.
 *
 *  Сокращённый набор Коммандер импортировать отказывался: в шаблоне
 *  «Заголовок 1», «Заголовок 2» и «Текст» встречаются ДВАЖДЫ — сначала
 *  поля обычного объявления, потом одноимённые поля комбинаторного.
 *  Получив одно такое имя, Коммандер брал последнее совпадение и считал
 *  строки комбинаторными объявлениями — отсюда «строки 1-37
 *  интерпретированы как комбинаторное». Позиция столбца тут значит
 *  больше названия, поэтому отдаём всю сетку целиком.
 */
const HEADERS = [
  ...
  'Вертикальное видео 1', 'Вертикальное видео 2', 'Квадратное видео 1',
  'Квадратное видео 2', 'Горизонтальное видео 1', 'Горизонтальное видео 2',
...
const COL = {
  ...
  url: 35, region: 37, bid: 38, bidNet: 39, negatives: 53,
} as const;
