 ...
 *  охвата: на части площадок вместо картинки крутится ролик.
 *
 *  ВАЖНО: в файл импорта эти ссылки не идут. Колонки «Вертикальное/
 *  квадратное/горизонтальное видео» ждут ID ролика из библиотеки Директа,
 *  а не адрес файла — ссылку Коммандер не примет. Ролики сначала грузят
 *  в кабинете. Функция нужна, чтобы знать адреса готовых файлов
 *  в /public/ads для этой загрузки. */
  ...
  const all =
    audience === 'worker'
      ? [
          ...
          { title: 'Оплата', desc: 'Стоимость работ обсуждаете с заказчиком напрямую', url: link('/', 'pay', '#practice') },
...
export const CALLOUTS: Record<'customer' | 'worker', string[]> = {
  ...
  worker: ['Ремонт, уборка, переезды', 'Без резюме', 'Сервис бесплатный', 'Заказы рядом'],
};
...
const COL = {
  ...
  slTitles: 22, slDescs: 23, slUrls: 24, callouts: 25,
  url: 26, region: 28, bid: 30, bidNet: 31, negatives: 32,
} as const;
...
/** Первые заголовки. Директ требует, чтобы в объявлении о заработке был
 *  назван конкретный вид работ, а не абстрактная «подработка». Поэтому
 *  почти каждый заголовок уточняет: ремонт, уборка, переезды, сборка
 *  мебели. Слова «шабашка» и «халтура» остаются в ключевых фразах —
 *  люди так ищут, — но в самих объявлениях их нет: для модерации они
 *  звучат как обещание лёгких денег. */
const workerTitle = (cityPrep: string): string[] => {
  const short = (v: string, fallback: string) =>
    v.length <= LIMITS.title ? v : fit(fallback, LIMITS.title);
  return [
    short(`Подработка в ${cityPrep}: ремонт, уборка`, 'Подработка: ремонт, уборка'),
    short(`Разовые заказы в ${cityPrep}`, 'Разовые заказы рядом'),
    short(`Работа на день в ${cityPrep}`, 'Работа на один день'),
    short(`Заказы на ремонт в ${cityPrep}`, 'Заказы на ремонт рядом'),
    short(`Переезды и грузчики в ${cityPrep}`, 'Заказы для грузчиков'),
    short(`Работа без резюме в ${cityPrep}`, 'Работа без резюме'),
    short(`Уборка и мелкий ремонт в ${cityPrep}`, 'Уборка и мелкий ремонт'),
  ];
};

/** Вторые заголовки. Модерация Директа проверяет объявления о заработке
 *  отдельно: нельзя обещать доход, гарантировать заказы и говорить, что
 *  всё легко и без рисков. Поэтому здесь только проверяемые факты про
 *  сервис — что он бесплатный и как устроен вход. Формулировки вида
 *  «Заказы каждый день» убраны: сервис не может это гарантировать. */
const workerTitle2 = (): string[] => [
  'Ремонт, уборка, переезды',
  'Без резюме и собеседований',
  'Заказы рядом с домом',
  'Сервис бесплатный для мастеров',
  'Вы сами выбираете заказы',
  'Заказчик платит напрямую',
  'Регистрация за минуту',
];

/** Тексты. Называем конкретный вид работ — ремонт, уборка, переезды,
 *  сборка мебели: требование Директа к объявлениям о заработке. Сумм
 *  и обещаний дохода нет, сервис их и не может дать — цену исполнитель
 *  обсуждает с заказчиком сам. */
const workerText = (cityPrep: string): string[] => [
  pickText2(
    `Заказы на ремонт, уборку, переезды и сборку мебели в ${cityPrep}. Оплату обсуждаете с заказчиком.`,
    `Заказы на ремонт, уборку и переезды. Оплату обсуждаете с заказчиком.`,
  ),
  pickText2(
    `Доска объявлений о разовых работах. Без резюме и собеседований, комиссию с оплаты не берём.`,
    `Доска объявлений о разовых работах. Без резюме, комиссию не берём.`,
  ),
  pickText2(
    `Заявки от жителей ${cityPrep}: сантехника, электрика, грузчики. Выбираете, что вам подходит.`,
    `Заявки от жителей: сантехника, электрика, грузчики. Выбираете сами.`,
  ),
];
